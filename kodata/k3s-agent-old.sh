...
checkK3SInstalled()
{
    info 'start check agent is installed 检测k3s agent是否已安装'
...
k3sInstall(){
   info "start registry agent node"
   curl -sfL https://rancher-mirror.rancher.cn/k3s/k3s-install.sh | K3S_URL=${K3S_URL} K3S_TOKEN=${K3S_TOKEN} INSTALL_K3S_MIRROR=cn INSTALL_K3S_SKIP_SELINUX_RPM=true INSTALL_K3S_SELINUX_WARN=true INSTALL_K3S_MIRROR_URL=rancher-mirror.rancher.cn \
   sh -s - 
}
