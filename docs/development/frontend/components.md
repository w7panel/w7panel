...
组件文档用于记录可复用组件的外部契约。开发时先判断组件是否跨页面复用：跨页面复用或被父组件通过 `ref` 调用的组件需要在本文记录 Props、Events、Ref 方法和使用注意；页面私有组件只在对应页面内维护即可。

### 基本流程

1. 新增公共组件时放到 `src/components/`，并在本文补充用途、Props、Events 和示例。
2. 修改组件外部参数、事件名、事件参数或 `ref` 方法时，同步更新本文。
3. 涉及 Wujie 微应用事件的组件，只在本文记录组件职责，事件协议维护到 [wujie-events.md](./wujie-events.md)。
