...
- 已安装 **AI 代理** 网关插件；如果尚未安装，AI 代理列表页会显示安装提示并禁用“新增”；
- 如需自动 HTTPS，域名必须已正确解析，且集群中的 `w7-letsencrypt-prod` ClusterIssuer 可用。

## 创建域名

1. 进入 **网关管理 → AI代理**。
2. 如果页面提示 AI 代理插件未安装，先点击 **安装 AI 代理插件** 完成制品安装。
3. 点击 **新增**，填写访问域名并按需启用 HTTPS 证书。
4. 创建完成后，在列表中点击 **编辑** 进入该域名的配置页。
...
- **模型名称**：在域名路由配置中输入该域名支持的模型名称。配置后，请求体中的 `model` 必须在列表内，否则网关返回 HTTP 403；留空表示不限制模型。如果请求校验插件尚未安装，输入框保持禁用，可在该栏点击 **立即安装**。
- **认证**：启用后至少要创建一个消费者。消费者区域支持添加多个消费者，并展示消费者名称和认证方式。每个消费者可以配置多个认证令牌及独立的令牌来源。如果 Key Auth 认证插件尚未安装，开关保持禁用，可在该表单项点击 **立即安装**；无插件安装权限时请联系管理员。

进入 AI 代理列表时，页面会按当前命名空间和 Ingress 名自动创建或开启对应的 AI 域名插件规则；域名编辑页无需再手动开启 AI 代理。AI Proxy、Key Auth 和请求校验的域名级规则统一精确关联 `namespace/ingressName`，不按 Host 域名匹配，也不读取或迁移旧 `domain`/裸 Ingress 规则。删除 AI 代理域名时会一并清理三个关联插件中 Host 相同的旧 `domain` 规则；其他升级遗留规则仍需管理员手工清理。

页面只读取 Ingress 中的 Provider 引用、域名作用域 Provider ID，以及消费者 Secret 的 `values`、`source`、`tokenKey`。旧 Provider Secret、旧 Provider ID/名称、消费者 Secret `key` 和 Key Auth 单数 `credential` 不再识别，升级前需要转换或清理这些历史资源。
